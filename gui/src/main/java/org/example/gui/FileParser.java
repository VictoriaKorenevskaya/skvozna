...
public class FileParser {
    ...
    public static void ReadInputFile(Settings InputFileSettings) throws Exception {
        if (InputFileSettings.check_file_zip && InputFileSettings.check_file_encrypted) {
            if (Objects.equals(InputFileSettings.file_type, ".txt")) {
                ...
                InputFileSettings.byte_info = Encryption.DecryptBytes(InputFileSettings.key, InputFileSettings.byte_info);
            ...
            if (Objects.equals(InputFileSettings.file_type, ".txt")) {
                InputFileSettings.txt_info = Encryption.DecryptFileToString(InputFileSettings.key, InputFileSettings.file_name);
                InputFileSettings.byte_info = Encryption.DecryptFileToBytes(InputFileSettings.key, InputFileSettings.file_name);
            } else if (Objects.equals(InputFileSettings.file_type, ".json")) {
                InputFileSettings.txt_info = Encryption.DecryptFileToString(InputFileSettings.key, InputFileSettings.file_name);
    ...
    public static void WriteOutputFile(Settings OutputFileSettings) throws Exception {
        if (OutputFileSettings.check_file_zip && OutputFileSettings.check_file_encrypted) {
            Archive.writeBytesToArchive(Encryption.EncryptBytes(OutputFileSettings.key, OutputFileSettings.byte_info), OutputFileSettings.archive_name, OutputFileSettings.file_name);
            ...
            if (Objects.equals(OutputFileSettings.file_type, ".txt")) {
                Operations.WriteBytesToFile(Encryption.EncryptBytes(OutputFileSettings.key, OutputFileSettings.byte_info), OutputFileSettings.file_name);
            } else if (Objects.equals(OutputFileSettings.file_type, ".json")) {
                JsonParser parser = new JsonParser();
